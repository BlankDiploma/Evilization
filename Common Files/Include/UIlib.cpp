...
void UIInstance::Render()
{
	...
	if (pDef->eType == kBox_Text || pDef->eType == kBox_Button)
	{
		...
		if (pchText)
		{
			...
			g_Renderer.AddStringToRenderList(pTextDef->hFont.pTex, pchText, x, y, pTextDef->textcolor, pTextDef->textFlags & kUITextFlag_CenterX, false, pTextDef->textFlags & kUITextFlag_DropShadow); 
